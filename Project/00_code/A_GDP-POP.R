...
GDP_Pop_Europe_correct <- rbind(GDP_extra_final, GDP_EU_final) %>%
  ...
  mutate(GDP_growth_09 = (GDP_capita_09 - lag(GDP_capita_09)) / lag(GDP_capita_09),
         ...
         # across(-c(Name, Year, GDP_growth_09), lag)) %>%
  filter(!NUTS %in% c("AL01", "AL02", "AL03")) %>%
  ungroup() %>%
  select(NUTS, Name, Year, GDP_EUR, GDP_EUR_2009, GDP_capita_09, GDP_growth_09, Population_abs, Pop_growth) %>%
  rename(GDP_EUR_to_keep = GDP_EUR_2009,
