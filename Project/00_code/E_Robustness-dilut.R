...
dilut_base2 = bms(datas_base[, !names(datas_base) %in% interaction], burn=3e+06, iter=10e+06,
                 g="BRIC", mprior="dilut", mcmc="bd",
                 user.int=TRUE, force.full.ols = TRUE, fixed.reg = YF)

dilut_base3 = bms(datas_base, burn=3e+06, iter=10e+06,
...
dilut_fix2 = bms(datas_fix, burn=3e+06, iter=10e+06, g="BRIC", mprior="dilut", mcmc="bd", 
                user.int= TRUE, force.full.ols = TRUE, fixed.reg = TF)
...
dilut_spat2 = spatFilt.bms(X.data = datas_spat[,!names(datas_spat) %in% interaction], WList = WL_decade_ext, 
                          burn = 3e+06,iter = 10e+06,
                          nmodel=100, mcmc="bd", g="BRIC", 
                          mprior="dilut", user.int = TRUE)

dilut_spat3 = spatFilt.bms(X.data = datas_spat, WList = WL_decade_ext, 
                          burn = 3e+06,iter = 10e+06,
