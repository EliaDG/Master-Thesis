...
data_EU <- full_join(ardeco, eurostat) %>%
  ...
  select(1:3, Migration_abs, Labor_Productivity_abs, Wage_EUR, Wage_NCU, Activity_rate, 48:75)
...
data_EXTRA <- extra %>%
  ...
  select(1:3, Activity_rate, Wage_EUR, Wage_NCU, NEET_share, Migration_abs, GFCF_share, 49:75)

dataset <- full_join(data_EU, data_EXTRA) %>%
  arrange(NUTS, Year) %>% 
  mutate(Country = sapply(NUTS, mapping_nuts)) %>% 
  select(Country, -Wage_NCU, -Unemp_edu_1_rate, -Unemp_edu_2_rate, -Unemp_edu_3_rate, everything()) 
